...
export default function Home({ onProductSelect, searchQuery = '' }: HomeProps) {
    ...
    // ✅ POLLING MIGRATION: Replaced Realtime with polling (more reliable)
    // Refreshes listings every 30s when tab is active
    useEffect(() => {
        console.log('⏱️ [HOME] Setting up smart polling for listings (30s, pause when inactive)...');

        const pollListings = () => {
            // Only poll if tab is active (smart polling!)
            if (document.hidden) {
                console.log('💤 [HOME] Tab inactive, skipping poll');
                return;
            }

            console.log('🔄 [HOME] Polling for listing updates...');
        ...
        const pollingInterval = setInterval(pollListings, 30000); // 30s

        // Also poll when tab becomes active again
        const handleVisibilityChange = () => {
            if (!document.hidden) {
                console.log('👀 [HOME] Tab active again, polling immediately...');
                pollListings();
            }
        };
    ...
    return (
        <>
            {/* SEO Meta Tags */}
            <Helmet>
                <title>Dekonvi - Petites Annonces Gratuites au Togo | Achat Vente</title>
                <meta name="description" content="Achetez et vendez gratuitement au Togo sur Dekonvi. Voitures, immobilier, emploi, téléphones, meubles et plus. La marketplace #1 du Togo." />
                <link rel="canonical" href="https://dekonvi.com" />
            </Helmet>
            ...
            <script type="application/ld+json">
                {JSON.stringify({
                    ...
                    "@type": "WebSite",
                    "name": "Dekonvi",
                    "alternateName": "Dekonvi Togo",
                    "url": "https://dekonvi.com",
                    "description": "Marketplace de petites annonces gratuites au Togo",
                    "potentialAction": {
                        "@type": "SearchAction",
                        "target": "https://dekonvi.com/?search={search_term_string}",
            ...
            <script type="application/ld+json">
                {JSON.stringify({
                    ...
                    "name": "Dekonvi",
                    "url": "https://dekonvi.com",
                    "logo": "https://dekonvi.com/logo.png",
                    "sameAs": [
                        "https://www.facebook.com/dekonvi",
                        "https://www.instagram.com/dekonvi"
                    ],
                    "contactPoint": {
                        "@type": "ContactPoint",
